...
// === Начало: Здесь обычно может быть код для проверки PWA/устройства,
// который, если найден, можно было бы удалить.
// Например: if (!window.matchMedia('(display-mode: standalone)').matches || !/mobile/i.test(navigator.userAgent)) { ... }
// Но в предоставленном тобой коде его нет.
// === Конец
